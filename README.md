...
- 左侧栏内嵌 **DeepSeek 网页版**（`chat.deepseek.com`）便利面板，网页会话内容按天归档到本地（见下方声明）
...
| 版本 | 特性 |
| --- | --- |
| **0.8.1-rc.1**（当前，Latest） | 内嵌 **DeepSeek 网页版面板**（`chat.deepseek.com`，免开浏览器）+ 会话内容按天归档到 `~/.dsh/web-chat/`；设置新增「会话版本」管理（list / probe / convert / hide / show）；升级可先下载后安装；见**声明：禁止反向代理**。内核 0.1.5-rc.1 |
